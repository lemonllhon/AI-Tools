...
### 变更
- 应用已重命名为 **AI Lemon Tools**。
- 默认导航聚焦 Codex、仪表盘和设置。
- 非 Codex 平台页面默认隐藏，并可通过平台显示配置重新开启。
- 顶部推广区域已替换为欢迎信息，不再展示选购或推广内容。

### 新增
- 新增 AI Lemon Tools 应用图标，并同步到前端与 Tauri 打包资源。
- 新增基于 Git 标签发布 GitHub Releases 的发布流程文档。
